...
export default function RoomCard({ roomType, onBook, disabled = false }: RoomCardProps) {
  ...
  return (
    <Card elevation={4} sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, mb: 3, borderRadius: 2, overflow: 'hidden', border: '2px solid', borderColor: 'primary.main' }}>
      ...
      <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
        ...
        <Box sx={{ bgcolor: 'action.hover', p: 1.5, borderRadius: 1, border: '1px dashed grey', mt: 'auto' }}>
          ...
          <Stack direction="row" flexWrap="wrap" gap={1} sx={{ mt: 1 }}>
            {roomType.numPcs > 0 && (
               <Chip icon={<ComputerIcon />} label={`${roomType.numPcs}x Lvl ${roomType.levelOfPc} PC`} size="small" color="primary" />
            )}
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2, borderLeft: { md: '1px solid #e0e0e0' } }}>
        ...
        >
          {disabled ? "Select Dates" : "Book a stay!"} 
        </Button>
      </Box>
